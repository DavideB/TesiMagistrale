...
xSpark uses a heuristic to compute per-stage deadlines and to estimate how many cores must be allocated for a stage to successfully fulfill the deadline. See \MySec{sec:heuristic} for an exhaustive description of its functionality.
...
\end{figure}
Furthermore, each single json profile is enhanced with information about the jobs composing the application, as shown in \MyFig{fig:profile_jobs_info}.

\begin{figure}[t]
	%\vspace{-0.5cm}
	\centering
	%\includegraphics[width=12cm]{images/xsparksymb_profile_jobs_info}
	\begin{minipage}{0,65\columnwidth}
		\lstinputlisting[
		firstline=1,
		lastline=45,		%float=tb,
		language=Python,
		tabsize=2,
		basicstyle=\tiny,
		numbers=left,
		...
		]{CodeFiles/profileJobsFragment.json}  
...
A \textit{Profiling} (see \MyFig{fig:xSparkExecFlow}) of the application is done by running it with each \textit{Launcher}'s set of arguments. Each profiling run generates the corresponding \plan in a specialized JSON file. At the end of this process, all the generated \plans are packaged into another JSON file, in jargon called \textit{JumboJSON}, to form the \model. All the generated JSON files are then stored along with the \model in the \textit{\model{ Store}} on the \textit{Spark Master} server. 
\lstinputlisting[
firstline=1,
lastline=33,
%float=thb,
...
\paragraph{Stage Deadline Adjustment}\label{sec:stage_deadline_adjustment}
...
stepnumber=1,
numbersep=5pt,
caption={Changes to case class SparkStageWeightSubmitted of class SparkListener.scala}, 
captionpos=t,
...
As it will be presented in \MyChap{chap:Evaluation} dedicated to the evaluation of the solution, we have to be able to run the applications under xSpark configured to use heuristic \texttt{HeuristicControlUnlimited} and providing a single \plan corresponding to the \textbf\textit{best case} (i.e. lowest number of stages) and the \textbf\textit{worst case} (i.e. highest number of stages). These test settings are necessary to compare the performance of xSpark with and without \textit{Symbolic Execution} capabilities, however this situation leads  to the problem that we call \textit{Stage Cardinality Mismatch}. It happens when the number of stages of the application does not match the number of stages in the \plan. We can have two cases: the \plan contains more stages than the application or, vice-versa, the \plan contains less stages than the application. In the former case (\MyFig{fig:stage_cardinality_mismatch_ok}) there are no problems, and  xSpark terminates correctly the execution of the application, while in the latter case xSpark tries to  retrieve information from the \plan about a non-existant stage and it terminates the execution of the application in error. To avoid this unwanted situation, we have changed the code in method \texttt{submitMissingTasks} of class \texttt{DAGScheduler.scala} shown in \MyListing{lst:DAGScheduler_submitMissingTasks}, where we added the variable \texttt{highestStageIdInProfile} that is initialized with the cardinality of the stages in the \plan and its value assigned to the variable \texttt{stageId} if the \texttt{id} of the requested stage is higher than \texttt{highestStageIdInProfile}. This creates the effect called \textit{stage id extension} depicted in \MyFig{fig:stage_id_extension}.
\begin{figure}[t]
	\vspace{-1.5cm}
	%\hspace*{-4cm}
	\centering
...
\section{Intercepting Actions Results}\label{sec:running_jobs}
...
\begin{figure}[thbp]
	%\vspace{-0.5cm}
	\centering
	%\includegraphics[width=0.75\columnwidth]{images/xspark_dagsymb_credentials_template.pdf}
	\begin{minipage}{0,65\columnwidth}
		\lstinputlisting[
		firstline=1,
		lastline=11,	%float=tb,
		language=Python,
		tabsize=2,
		basicstyle=\small,
		numbers=left,
		...
		caption=, nolol,
		%captionpos=t,
		%label=lst:codefilecredentialstemplate
		]{CodeFiles/credentials_template.json}  
	\end{minipage}
...
\begin{figure}[t]
	\vspace{0.5cm}
	\centering
	%\includegraphics[width=12.5cm]{images/xspark_dagsymb_setup.pdf}
	\begin{minipage}{0,65\columnwidth}
		\lstinputlisting[
		firstline=1,
		lastline=42,	%float=tb,
		language=Python,
		tabsize=2,
		basicstyle=\tiny,
		numbers=left,
...
For each experiment the  \textbf{average\_runs.py} module is called to create a JSON profile called  <\emph{appname}>.json containing the average values of the $n$ \plans obtained by iterating the profiling $n$ times. 
...
%\href{https://github.com/gioenn/xSpark.git}{xSpark}
, and require the
...
\begin{verbatim}
$ python3 xSpark_dagsymb.py profile_symex -r 5 
  CallsExample_1.json CallsExample_2.json
\end{verbatim}

Run the Application Test with 5 iterations:

\begin{verbatim}
$ python3 xSpark_dagsymb.py submit_symex  -r 5 
  CallsExample_1.json CallsExample_2.json
\end{verbatim}
\end{enumerate}