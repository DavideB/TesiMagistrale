...
\paragraph{Adapting Weigth Calculation to Multiple \plans}
Selecting a new \plan at each job boundary for the execution of the upcoming job means that the scheduler has to deal with a situation where the previous stages of the application were executed under a different \plan than the newly selected one. The new \plan, in general, has a  different number of stages and different durations for each stage. When a stage is submitted for execution, the \texttt{heuristic} calculates its stage deadline $deadline(sk)$ and the stage related weights $w1(sk)$, $w2(sk)$ and $weigth(sk)$, as explained in \MySec{sec:heuristic}.
...
\section{\tool Python Tool}\label{sec:xspark_dagsymb}
