...
To run the experiments  we slightly modified the test cases generated by \dSymb to increase the size of the datasets (without breaking the path conditions). This way, we ensure to test the desired paths and reliably obtain different repetitions of the experiments. However, the datasets were not created completely randomly. For this reason, we conducted some preliminary experiments with datasets created completely randomly and obtained results similar to those presented. A broader set of experiments could be done to address this aspect as a matter for future developments.
